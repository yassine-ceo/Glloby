...
import { getAuth, onAuthStateChanged, signInWithEmailAndPassword, createUserWithEmailAndPassword,
         signInAnonymously, sendEmailVerification, signOut }
    from "https://www.gstatic.com/firebasejs/10.8.1/firebase-auth.js";
import { getFirestore, doc, getDoc, setDoc, updateDoc, collection, query, orderBy,
         limit, onSnapshot, addDoc, serverTimestamp, where }
    from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
import { getDatabase, ref, onValue, set, onDisconnect, serverTimestamp as rtdbTs }
...
// ── STATE ──
let currentUser = null; // Firestore profile map
let cityId = "Rabat";
...
const popupModal   = $("user-profile-popup");
...
window.addEventListener("popstate", () => {
    ...
    if (VIEWS.includes(p)) showView(p);
});

// ── PREMIUM GATE ──
document.querySelectorAll(".premium-gate").forEach(el =>
    el.addEventListener("click", e => { e.preventDefault(); openModal(premiumModal); }));
...
onAuthStateChanged(auth, async user => {
    if (!user) { openModal(authModal); setAuthStep("auth-step-login"); stopApp(); return; }
    if (!user.emailVerified && !user.isAnonymous) { openModal(authModal); setAuthStep("auth-step-verify"); return; }
    const snap = await getDoc(doc(db, "users", user.uid));
    if (!snap.exists() || !snap.data().name) { openModal(authModal); setAuthStep("auth-step-setup"); return; }
...
$("btn-login").addEventListener("click", async () => {
    ...
    } catch(e) { setErr("auth-error", friendlyError(e)); }
...
$("btn-register").addEventListener("click", async () => {
    ...
    try {
        ...
        $("verify-email-msg").textContent = `Verification sent to ${email}`;
...
$("btn-check-verify").addEventListener("click", async () => {
    ...
    if (auth.currentUser?.emailVerified) setAuthStep("auth-step-setup");
    else setErr("verify-error","Not verified yet. Check your inbox.");
});
...
// PFP preview
$("setup-pfp-input").addEventListener("change", e => {
    const f = e.target.files[0];
    if (!f) return;
    selectedPfpFile = f;
    const url = URL.createObjectURL(f);
    $("setup-avatar-img").src = url;
    $("pfp-status").textContent = "✓ Photo selected";
});
$("setup-name").addEventListener("input", () => {
    const n = $("setup-name").value.trim();
    if (n) $("setup-avatar-img").src =
        `https://ui-avatars.com/api/?name=${encodeURIComponent(n)}&background=3b82f6&color=fff&size=200`;
});

// Complete setup
$("btn-complete-setup").addEventListener("click", async () => {
    const name     = $("setup-name").value.trim();
    const username = $("setup-username").value.trim().replace(/\s/g,"");
    const age      = parseInt($("setup-age").value);
    const gender   = $("setup-gender").value;
    const city     = $("setup-city").value;
    const country  = $("setup-country").value.trim() || "Morocco";
    ...
    setErr("setup-error","");
    $("btn-complete-setup").innerHTML = "<span>Uploading…</span>";
    try {
        ...
        const profile = {
            ...
            is_online: true, lastSeen: serverTimestamp()
        };
        ...
        updateProfileUI();
        startApp();
    } catch(e) { setErr("setup-error", e.message); }
    $("btn-complete-setup").innerHTML = "<span>Go Live on Map</span><i class='bx bx-right-arrow-alt'></i>";
});
...
$("logout-btn").addEventListener("click", async () => {
    if (auth.currentUser) {
        await updateDoc(doc(db, "users", auth.currentUser.uid),
            { is_sharing_location: false, is_online: false, lastSeen: serverTimestamp() }).catch(()=>{});
    }
    await signOut(auth);
});
...
function setupPresence() {
    ...
    onValue(connRef, snap => {
        if (!snap.val()) return;
        set(statusRef, { status:"online", last_online: null });
        onDisconnect(statusRef).set({ status:"offline", last_online: rtdbTs() });
    });
}
...
async function initMap() {
    ...
    $("fab-chat").click; // handled by premium-gate
...
async function syncMyLocation([lat,lng]) {
    ...
    await updateDoc(doc(db,"users",uid),{
        location:{ latitude:lat, longitude:lng, timestamp:serverTimestamp() },
        is_sharing_location:true
    }).catch(()=>{});
}

function listenToUsers() {
    const q = query(collection(db,"users"), where("is_sharing_location","==",true), limit(200));
...
// ── USER PROFILE POPUP ──
$("close-popup").addEventListener("click", () => closeModal(popupModal));
...
$("popup-chat-btn").addEventListener("click", () => {
...
function initChat() {
    ...
    const q = query(
        collection(db,"cityChats",cityId,"messages"),
...
async function sendChatMsg() {
    ...
    $("chat-input").value = "";
    await addDoc(collection(db,"cityChats",cityId,"messages"), {
...
$("chat-input").addEventListener("keypress", e => e.key==="Enter" && sendChatMsg());

function listenOnlineCount() {
    const q = query(collection(db,"users"), where("city","==",cityId), where("is_online","==",true));
    onSnapshot(q, snap => {
...
function handleRoute() {
    const p = location.pathname.replace("/","") || "map";
    if (VIEWS.includes(p)) showView(p); else showView("map");
}
